...
vector<VkVertexInputBindingDescription> VeModel::Vertex::getBindingDescriptions(
	void) {
...
VeModel::Vertex::getAttributeDescriptions(void) {
	...
	while (getline(file, line)) {
		...
		if (token == "v") {	 // Vertex
			...
			for (int i = 0; i < 4; i++) {
				...
				if (vertexIndices.empty()) break;
				if (i == 3) {
					indices.push_back(x[0]);
					indices.push_back(x[2]);
				}

				istringstream indicesStream(
					vertexIndices);	 // Make it a input string stream
				found =
					vertexIndices.find('/');  // Find if face have uv and normal
				...
				if (uv >= 0) {
					...
					if (i == 0)
						vertex.uv = vem::vec2(0.f, 1.f);
					else if (i == 1)
						vertex.uv = vem::vec2(0.f, 0.f);
					else if (i == 2)
						vertex.uv = vem::vec2(1.f, 0.f);
					else if (i == 3)
						vertex.uv = vem::vec2(1.f, 1.f);
				}
				...
				if (color == 0) {
					if (si == 0)
						vertex.color = vem::vec3(1.f, 0.f, 0.f);
					else if (si == 1)
						vertex.color = vem::vec3(0.f, 1.f, 0.f);
					else if (si == 2)
						vertex.color = vem::vec3(0.f, 0.f, 1.f);
					else if (si == 3)
						vertex.color = vem::vec3(0.f, 1.f, 1.f);
					else if (si == 4)
						vertex.color = vem::vec3(1.f, 0.f, 1.f);
					else if (si == 5)
						vertex.color = vem::vec3(1.f, 1.f, 0.f);
				} else if (color == 1) {
					if (si == 0)
						vertex.color = vem::vec3(.125f);
					else if (si == 1)
						vertex.color = vem::vec3(.250f);
					else if (si == 2)
						vertex.color = vem::vec3(.375f);
					else if (si == 3)
						vertex.color = vem::vec3(.500f);
					else if (si == 4)
						vertex.color = vem::vec3(.625f);
					else if (si == 5)
						vertex.color = vem::vec3(.875f);
				} else {
			...
			if (normal < 0) {
				vem::vec3 tmp =
					vem::vec3(vertices[indices[indices.size() - 1]].position +
...
void VeModel::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width,
								uint32_t height) {
...
void VeModel::transitionImageLayout(VkImage image, VkFormat format,
...
void VeModel::createImage(uint32_t width, uint32_t height, VkFormat format,
	...
	if (vkAllocateMemory(_veDevice.device(), &allocInfo, nullptr,
						 &imageMemory) != 0)
...
unsigned char *loadImage(const char *filename, int *x, int *y, int *comp,
						 int req_comp) {
...
void VeModel::createTextureImages(const int &texture) {
	...
	string filename;
	if (texture == 0)
		filename = "model/texture/LolinEagle.png";
	else if (texture == 1)
		filename = "model/texture/Stone.png";
	else
		filename = "model/texture/Wood.png";
	unsigned char *pixels =
		loadImage(filename.c_str(), &texWidth, &texHeight, &texChannels, 4);

	if (!pixels) throw(runtime_error("image failed to load"));
	...
	_veDevice.createBuffer(imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
						   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
							   VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
						   stagingBuffer, stagingBufferMemory);
	vkMapMemory(_veDevice.device(), stagingBufferMemory, 0, imageSize, 0,
				&data);
...
void VeModel::createTextureImageView(void) {
	...
	if (vkCreateImageView(_veDevice.device(), &viewInfo, nullptr,
						  &_textureImageView) != 0)
...
void VeModel::createTextureSampler(void) {
	...
	if (vkCreateSampler(_veDevice.device(), &samplerInfo, nullptr,
						&_textureSampler) != 0)
...
void VeModel::createVertexBuffers(const vector<Vertex> &vertices) {
	...
	VeBuffer stagingBuffer{_veDevice, vertexSize, _vertexCount,
	...
	_vertexBuffer = make_unique<VeBuffer>(
		_veDevice, vertexSize, _vertexCount,
		VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
		VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
	_veDevice.copyBuffer(stagingBuffer.getBuffer(), _vertexBuffer->getBuffer(),
						 bufferSize);
}
...
void VeModel::createIndexBuffers(const vector<uint32_t> &indices) {
	...
	VeBuffer stagingBuffer{_veDevice, indexSize, _indexCount,
						   VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
						   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
							   VK_MEMORY_PROPERTY_HOST_COHERENT_BIT};
	...
	_indexBuffer = make_unique<VeBuffer>(
		_veDevice, indexSize, _indexCount,
		VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
		VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
	_veDevice.copyBuffer(stagingBuffer.getBuffer(), _indexBuffer->getBuffer(),
						 bufferSize);
}

unique_ptr<VeModel> VeModel::createModelFromFile(VeDevice &device,
