...
void VeRenderer::createCommandBuffers(void) {
	...
	if (vkAllocateCommandBuffers(_veDevice.device(), &allocInfo,
								 _commandBuffers.data()) != 0)
		throw(runtime_error("failed to allocate command buffers"));
}

void VeRenderer::freeCommandBuffers(void) {
	vkFreeCommandBuffers(_veDevice.device(), _veDevice.getCommandPool(),
						 static_cast<uint32_t>(_commandBuffers.size()),
...
void VeRenderer::recreateSwapChain(void) {
	...
	if (_veSwapChain == nullptr) {
		...
		_veSwapChain =
			make_unique<VeSwapChain>(_veDevice, extent, oldSwapChain);
...
void VeRenderer::endFrame(void) {
	...
	auto result =
		_veSwapChain->submitCommandBuffers(&commandBuffer, &_currentImageIndex);
...
void VeRenderer::beginSwapChainRenderPass(VkCommandBuffer commandBuffer) {
	...
	renderPassInfo.framebuffer =
		_veSwapChain->getFrameBuffer(_currentImageIndex);
	...
	viewport.y = 0.0f;
	viewport.width =
		static_cast<float>(_veSwapChain->getSwapChainExtent().width);
	viewport.height =
		static_cast<float>(_veSwapChain->getSwapChainExtent().height);
