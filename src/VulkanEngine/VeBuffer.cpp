...
VkDeviceSize VeBuffer::getAlignment(VkDeviceSize deviceSize,
									VkDeviceSize minOffsetAlignment) {
	if (minOffsetAlignment > 0)
		return ((deviceSize + minOffsetAlignment - 1) &
...
VeBuffer::VeBuffer(VeDevice &device, VkDeviceSize instanceSize,
	...
	device.createBuffer(_bufferSize, _usageFlags, _memoryPropertyFlags, _buffer,
						_memory);
}
...
void VeBuffer::writeToBuffer(void *data, VkDeviceSize size,
							 VkDeviceSize offset) {
...
VkDescriptorBufferInfo VeBuffer::descriptorBufferInfo(VkDeviceSize size,
													  VkDeviceSize offset) {
	return (VkDescriptorBufferInfo{_buffer, offset, size});
}

VkBuffer VeBuffer::getBuffer(void) const { return (_buffer); }
