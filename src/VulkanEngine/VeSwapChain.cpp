...
void VeSwapChain::createSwapChain(void) {
	...
	if (vkCreateSwapchainKHR(_device.device(), &createInfo, nullptr,
							 &_swapChain) != 0)
...
void VeSwapChain::createImageViews(void) {
	...
	for (size_t i = 0; i < _swapChainImages.size(); i++) {
		...
		if (vkCreateImageView(_device.device(), &viewInfo, nullptr,
...
void VeSwapChain::createDepthResources(void) {
	...
	for (int i = 0; i < _depthImages.size(); i++) {
		...
		_device.createImageWithInfo(imageInfo,
...
void VeSwapChain::createRenderPass(void) {
	...
	if (vkCreateRenderPass(_device.device(), &renderPassInfo, nullptr,
						   &_renderPass) != 0)
...
void VeSwapChain::createFramebuffers(void) {
	...
	for (size_t i = 0; i < imageCount(); i++) {
		array<VkImageView, 2> attachments = {_swapChainImageViews[i],
											 _depthImageViews[i]};
		VkExtent2D swapChainExtent = getSwapChainExtent();
...
void VeSwapChain::createSyncObjects(void) {
	...
	for (uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
		if (vkCreateSemaphore(_device.device(), &si, nullptr,
							  &_imageAvailableSemaphores[i]) != 0 ||
			vkCreateSemaphore(_device.device(), &si, nullptr,
							  &_renderFinishedSemaphores[i]) != 0 ||
			vkCreateFence(_device.device(), &fenceInfo, nullptr,
						  &_inFlightFences[i]) != 0)
			throw(runtime_error(
...
VkSurfaceFormatKHR VeSwapChain::chooseSwapSurfaceFormat(
	const vector<VkSurfaceFormatKHR> &format) {
...
VkPresentModeKHR VeSwapChain::chooseSwapPresentMode(
	const vector<VkPresentModeKHR> &present) {
...
VeSwapChain::VeSwapChain(VeDevice &deviceRef, VkExtent2D extent,
						 shared_ptr<VeSwapChain> previous)
...
VeSwapChain::~VeSwapChain() {
	...
	for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
		vkDestroySemaphore(_device.device(), _renderFinishedSemaphores[i],
						   nullptr);
		vkDestroySemaphore(_device.device(), _imageAvailableSemaphores[i],
						   nullptr);
...
VkRenderPass VeSwapChain::getRenderPass(void) { return (_renderPass); }

VkImageView VeSwapChain::getImageView(int index) {
	return (_swapChainImageViews[index]);
}

size_t VeSwapChain::imageCount(void) { return (_swapChainImages.size()); }

VkFormat VeSwapChain::getSwapChainImageFormat(void) {
	return (_swapChainImageFormat);
}

VkExtent2D VeSwapChain::getSwapChainExtent(void) { return (_swapChainExtent); }

uint32_t VeSwapChain::width(void) { return (_swapChainExtent.width); }

uint32_t VeSwapChain::height(void) { return (_swapChainExtent.height); }
...
VkResult VeSwapChain::acquireNextImage(uint32_t *imageIndex) {
	vkWaitForFences(_device.device(), 1, &_inFlightFences[_currentFrame],
					VK_TRUE, numeric_limits<uint64_t>::max());

	VkResult result = vkAcquireNextImageKHR(
		_device.device(), _swapChain, numeric_limits<uint64_t>::max(),
		_imageAvailableSemaphores[_currentFrame],  // Must be a not signaled
												   // semaphore
...
VkResult VeSwapChain::submitCommandBuffers(const VkCommandBuffer *buffers,
										   uint32_t *image) {
	if (_imagesInFlight[*image] != VK_NULL_HANDLE)
		vkWaitForFences(_device.device(), 1, &_imagesInFlight[*image], VK_TRUE,
						UINT64_MAX);
	...
	if (vkQueueSubmit(_device.graphicsQueue(), 1, &submitInfo,
					  _inFlightFences[_currentFrame]) != 0)
