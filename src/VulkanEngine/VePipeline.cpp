...
void VePipeline::createGraphicsPipeline(const string &vertFilepath,
	...
	if (vkCreateGraphicsPipelines(_veDevice.device(), VK_NULL_HANDLE, 1,
								  &pipelineInfo, nullptr,
								  &_graphicsPipeline) != 0)
		throw(runtime_error("failed to create graphics pipelines"));
}

void VePipeline::createShaderModule(const vector<char> &code,
									VkShaderModule *shaderModule) {
	VkShaderModuleCreateInfo createInfo{};
	createInfo.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
	createInfo.codeSize = code.size();
	createInfo.pCode = reinterpret_cast<const uint32_t *>(code.data());
	if (vkCreateShaderModule(_veDevice.device(), &createInfo, nullptr,
							 shaderModule) != 0)
		throw(runtime_error("failed to create shader module"));
}

VePipeline::VePipeline(VeDevice &device, const string &vertFilepath,
					   const string &fragFilepath,
					   const PipelineConfigInfo &configInfo)
	: _veDevice(device) {
