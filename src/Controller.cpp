...
bool Controller::getTextureOn(void) const { return (_textureOn); }

void Controller::moveInPlaneXZ(GLFWwindow *window, float dt,
							   VeGameObject &gameObject) {
	// Escape
	if (glfwGetKey(window, _keys.escape) == GLFW_PRESS)
	...
	if (newXpos < 0 || newYpos < 0 || newXpos > WIDTH ||
		newYpos > HEIGHT)  // If cursor not in window
		return;
	...
	if (vem::dot(rotate, rotate) > numeric_limits<float>::epsilon())
		gameObject._transform.rotation +=
			vem::vec3(_lookSpeed * dt) * vem::normalize(rotate);
	gameObject._transform.rotation.x =
		vem::clamp(gameObject._transform.rotation.x, -1.5f, 1.5f);
	gameObject._transform.rotation.y =
		vem::mod(gameObject._transform.rotation.y, TWO_PI);

	float yaw = gameObject._transform.rotation.y;
	const vem::vec3 forwardDir{sin(yaw), 0.f, cos(yaw)};  // Forward direction
	const vem::vec3 rightDir{forwardDir.z, 0.f,
							 -forwardDir.x};  // Right direction
	...
	if (vem::dot(moveDir, moveDir) > numeric_limits<float>::epsilon())
		gameObject._transform.translation +=
			vem::vec3(_moveSpeed * dt) * vem::normalize(moveDir);

	// Texture on
	if (glfwGetKey(window, _keys.texture) == GLFW_PRESS &&
