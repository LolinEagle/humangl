...
void PointLight::createPipelineLayout(VkDescriptorSetLayout globalSetLayout) {
	...
	if (vkCreatePipelineLayout(_veDevice.device(), &layoutInfo, nullptr,
							   &_pipelineLayout) != 0)
...
void PointLight::createPipeline(VkRenderPass renderPass) {
	...
	_vePipeline =
		make_unique<VePipeline>(_veDevice, "shader/pointLight.vert.spv",
								"shader/pointLight.frag.spv", pipelineConfig);
}

PointLight::PointLight(VeDevice &device, VkRenderPass renderPass,
...
void PointLight::update(FrameInfo &frameInfo, GlobalUbo &ubo) {
	auto rotateLight =
		vem::rotate(vem::mat4(1.f), frameInfo.frameTime, {0.f, -1.f, 0.f});
	int lightIndex = 0;

	for (auto &kv : frameInfo.gameObject) {
		auto &obj = kv.second;
		if (obj._pointLight == nullptr) continue;

		// Time
		obj._transform.translation =
			vem::vec3(rotateLight * vem::vec4(obj._transform.translation, 1.f));
		ubo.pointLights[lightIndex].position =
			vem::vec4(obj._transform.translation, 1.f);
		ubo.pointLights[lightIndex].color =
			vem::vec4(obj._color, obj._pointLight->lightIntensity);
		lightIndex++;
	}
...
void PointLight::render(FrameInfo &frameInfo) {
	...
	for (auto &kv : frameInfo.gameObject) {
		...
		auto offset =
			frameInfo.camera.getPosition() - obj._transform.translation;
	...
	vkCmdBindDescriptorSets(frameInfo.commandBuffer,
							VK_PIPELINE_BIND_POINT_GRAPHICS, _pipelineLayout, 0,
							1, &frameInfo.globalDescriptorSet, 0, nullptr);
	// Iterate through sorted lights in reverse order
	for (auto it = sorted.rbegin(); it != sorted.rend(); it++) {
		auto &obj = frameInfo.gameObject.at(
			it->second);  // Use obj id to find light object
		...
		vkCmdPushConstants(
			frameInfo.commandBuffer, _pipelineLayout,
